...
async function editAssetLink() {
  const current = currentProject.asset_link || '';
  let u = prompt('Asset link:', current);
  if (u === null) return;
  u = u.trim();
  if (u && !u.match(/^https?:\/\//)) u = 'https://' + u;
  await api("/projects/" + currentProject.id, { method: "PUT", body: JSON.stringify({ asset_link: u || null }) });
  currentProject.asset_link = u || null;
  renderProjectDetail();
}
...
function renderProjectDetail() {
  ...
  const assetUrl = p.asset_link ? (p.asset_link.match(/^https?:\/\//) ? p.asset_link : 'https://' + p.asset_link) : '';
  if (assetUrl) {
    meta.push(`<span>Asset: <a href="${esc(assetUrl)}" target="_blank" style="color:#1565c0">${esc(assetUrl.length > 40 ? assetUrl.substring(0, 40) + '...' : assetUrl)} &#8599;</a> <a href="#" onclick="editAssetLink();return false" style="color:#888;font-size:11px;text-decoration:none">edit</a></span>`);
  } else {
    meta.push(`<span>Asset: <a href="#" onclick="editAssetLink();return false" style="color:#1565c0;text-decoration:none">+ add link</a></span>`);
  }
...
async function loadEntryImages(entryId) {
  ...
  try {
    ...
    if (!images.length) {
      gallery.innerHTML = entry.image_path ? `<img src="${APP_BASE}/uploads/${entry.image_path}" onclick="showImage('${APP_BASE}/uploads/${entry.image_path}')" class="img-thumb">` : '<div class="no-img-sm">No img</div>';
      return;
    }
    gallery.innerHTML = images.map(img => `
      <div class="thumb-wrap">
        <img src="${APP_BASE}/uploads/${img.image_path}" onclick="showImage('${APP_BASE}/uploads/${img.image_path}')" class="img-thumb">
